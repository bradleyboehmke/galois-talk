...
2. ML interpretability
  ...
  - .bold.blue[Variable importance measures]: advance model-agnostic variable importance measures
...
--
      ...
      - <font style="color:red">Stacked models: ???</font>
--
   - Not comparable
   - Not intuitive
...
.center.bold[_Data scientists need better approaches to analyze feature important._]
...
.font110[
Then we measure importance based on flatness:
...
.font110[
And a simple measure flatness ( $F(\cdot)$ ) is standard deviation for continuous predictors and the range statistic for categorical variables:
...
```{r, echo=FALSE, warning=FALSE, fig.height=8, fig.width=5, fig.align='center'}
pdps <- lapply(features, FUN = function(feature) {
  pd <- partial(any_model, pred.var = feature)
  autoplot(pd) + 
    ylim(range(trn$y)) + 
...
as.character(methods(vi_model)) %>% 
  stringr::str_remove_all("vi_model\\.") %>% 
  tibble::as_tibble_col(column_name = "Models") %>%
